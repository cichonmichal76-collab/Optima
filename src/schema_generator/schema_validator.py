        ...
        if not draft.lines:
            issues.append(
                self._issue(
                    ...
                    draft,
                    "SCHEMA_EMPTY",
                    "Schemat nie zawiera żadnych pozycji.",
        ...
        if "WN" not in sides or "MA" not in sides:
            issues.append(
                self._issue(
                    ...
                    "Uzupełnij brakującą stronę dekretu.",
                )
            )
        ...
        if not draft.condition or draft.condition.strip().lower() in {"true", "1", "always"}:
            issues.append(
                self._issue(
                    ...
                    "Warunek schematu jest zbyt ogólny.",
                    "Doprecyzuj warunek, aby schemat nie łapał zbyt wielu dokumentów.",
                    confidence=0.85,
                )
            )
            ...
            if not line.account:
                issues.append(
                    self._issue(
                        ...
                        "Uzupełnij konto po stronie Wn lub Ma.",
                    )
                )
            if not line.amount_expression:
                issues.append(
                    self._issue(
                        ...
                        "Uzupełnij sposób wyliczenia kwoty.",
                    )
                )
        ...
        if "50" in draft.template_id and not any("vat" in (line.amount_expression or "").lower() for line in draft.lines):
            issues.append(
                self._issue(
                    ...
                    draft,
                    "SCHEMA_VAT_SPLIT",
                    "Schemat 50% VAT nie zawiera wyraźnego rozbicia kwoty VAT.",
                    "Zweryfikuj część odliczoną i nieodliczoną VAT.",
                    confidence=0.85,
                )
            )
