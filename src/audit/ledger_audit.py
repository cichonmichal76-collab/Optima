            ...
            if not (record.account_wn or record.account):
                issues.append(
                    self._issue(
                        ...
                        "Uzupełnij konto Wn albo pole Konto z eksportu Optimy.",
                    )
                )
            if not (record.account_ma or record.account_opposite):
                issues.append(
                    self._issue(
                        ...
                        "Uzupełnij konto Ma albo pole Konto przeciw. z eksportu Optimy.",
                    )
                )
            ...
            if not record.description:
                issues.append(
                    self._issue(
                        ...
                        "Brak opisu księgowania.",
                        "Uzupełnij opis lub popraw mapowanie kolumn.",
                    )
                )
                ...
                if period_start and record.accounting_date < period_start or period_end and record.accounting_date > period_end:
                    issues.append(
                        self._issue(
                            ...
                            "Data księgowania jest poza zakresem audytu.",
                ...
                if account and plan_accounts and account not in plan_accounts:
                    issues.append(
                        self._issue(
                            ...
                            "Zweryfikuj plan kont i dekretację dokumentu.",
                        )
                    )
                ...
                if account_pool and not (account_pool & vat_accounts):
                    issues.append(
                        self._issue(
                            ...
                            "Księgowanie VAT wykorzystuje inne konto niż zdefiniowane w konfiguracji.",
                            "Sprawdź konta VAT i szablon dekretacji.",
                            confidence=0.8,
                        )
                    )
            ...
            if abs(total_wn - total_ma) > self.tolerance:
                ...
                issues.append(
                    AuditIssue(
                        ...
                        recommendation="Zweryfikuj komplet dekretów oraz kwoty Wn/Ma.",
                        confidence=0.98,
                    )
                )
